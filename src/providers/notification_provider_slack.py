...
from providers.notification_provider import NotificationProvider, Notification
...
def respond_to_slack_within_3_seconds(ack, respond, command):
    ...
    if parser.type not in valid_commands:
        ack(f'Please enter a valid command. Example: {COMMAND} serp google.com "google+search+api" daily')
        return
...
class SlackCommandParser:
    ...
    def __post_init__(self):
        ...
        self.domain = self._search(rf'\|({REGEX_DOMAIN})>') if '|' in self.text else None 
        self.website = self._search(rf'<(https?://{REGEX_DOMAIN})>') if 'http' in self.text else None
